  ...
  namespace cxx
  {
    namespace tree
    {
      ...
      struct type_name_map
      {
        ...
        typedef void (*serializer) (xml::dom::element<C>&, const type&);
                   ...
                   const X& x) const
        {
          ...
          if (const type_info* ti = find_type (tid))
          {
            if (qualified)
            {
              ...
              xml::dom::attribute<C> a (
                xml::bits::type<C> (), xml::bits::xsi_namespace<C> (), e);

              a << qname<C> (ti->name ().namespace_ (), ti->name ().name ());
            }
            else
            {
              xml::dom::element<C> e (name, parent);
              ...
              if (const type_info* ti = find_substitution (i->second, tid))
              {
                ...
                if (ti->name ().name () != name ||
                  ...
                  throw unexpected_element<C> (
                    name, ns, ti->name ().name (), ti->name ().namespace_ ());
      ...
      namespace bits
      {
        ...
        {
          static void
          impl (xml::dom::element<C>& e, const type& x)
          {
