...
#include "/home/kde-devel/src/kdesupport/strigi/src/daemon/dbus/dbusserialization.h"
...
void
DBusXesamLiveSearchInterface::GetHitCountResponse(void* msg, uint32_t count) {
    DBusMessage* m = static_cast<DBusMessage*>(msg);
    DBusMessageWriter writer(conn, m);
    writer << count;
    dbus_message_unref(m);
...
void
DBusXesamLiveSearchInterface::GetHitDataResponse(void* msg,             const std::vector<std::vector<Strigi::Variant> >& hit_data) {
    DBusMessage* m = static_cast<DBusMessage*>(msg);
    DBusMessageWriter writer(conn, m);
    writer << hit_data;
    dbus_message_unref(m);
